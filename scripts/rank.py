...
def parse_arguments():
    ...
    parser.add_argument("--teacher_max_model_len", type=int, default=4096, help="Max model length for teacher LLM in vLLM")
...
def score_with_teacher_vllm(prompts, all_responses, llm, max_length=1024, batch_size=4, template_path=None, teacher_tokenizer=None, teacher_max_model_len=4096):
    ...
    for i in range(0, len(prompts), batch_size):
        ...
        for batch_idx, (prompt, responses) in enumerate(zip(batch_prompts, batch_responses)):
            for response_idx, response in enumerate(responses):
                scoring_prompt = create_scoring_prompt(prompt, response, responses, template_path=template_path)
                if teacher_tokenizer is not None and hasattr(teacher_tokenizer, 'apply_chat_template') and teacher_tokenizer.chat_template is not None:
                    scoring_prompt = apply_template(scoring_prompt, teacher_tokenizer)
                # truncate to avoid exceeding model length
                if teacher_tokenizer is not None:
                    try:
                        token_ids = teacher_tokenizer.encode(scoring_prompt, add_special_tokens=False)
                        limit = max(16, teacher_max_model_len - 16)
                        if len(token_ids) > limit:
                            token_ids = token_ids[:limit]
                            scoring_prompt = teacher_tokenizer.decode(token_ids, skip_special_tokens=True)
                    except Exception:
                        pass
...
def score_with_teacher_llm(prompts, candidates, teacher_model, batch_size=4, template_path=None, bt_method="bradley_terry_mle", tensor_parallel_size=1, teacher_tokenizer=None, teacher_max_model_len=4096):
    ...
    absolute_scores_list = score_with_teacher_vllm(prompts, candidates, llm, batch_size=batch_size, template_path=template_path, teacher_tokenizer=teacher_tokenizer, teacher_max_model_len=teacher_max_model_len)
...
def ranking(args, prompts, candidates, teacher_tokenizer=None):
...
def main(args):
    ...
    ranking(args, prompts_all, candidates_texts, teacher_tokenizer=teacher_tokenizer)
